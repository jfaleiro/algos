...
class LinearSGDRegression(Algorithm):
    ...
    def do_fit(self, features, responses):
        ...
        for _ in range(self.epochs):
            ...
            errors += [rmse(predictions[yhat_column].values, responses.values)]
        return errors

    def do_predict_slower(self, features):
        def yhat(row):
    ...
    def do_predict(self, features):
        bias_column = '_bias'
        features = features.copy()
        features[bias_column] = 0.0
        columns = sorted(features.columns)
        columns.remove(bias_column)
        columns = [bias_column] + columns
        coefficients_dict = dict(columns=columns,
                                 coefficient=self.coefficients)
        coefficients = pd.DataFrame(coefficients_dict).set_index('columns')
